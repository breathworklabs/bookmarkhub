...
class TwitterBookmarkExtractor {
  ...
  async initializeExtractor() {
    try {
      ...
      if (isLoggedIn) {
        console.log('Twitter Bookmark Extractor initialized - User is logged in');
        this.notifyPopup('ready', { loggedIn: true });
      } else {
        console.log('Twitter Bookmark Extractor initialized - User not logged in');
  ...
  async extractBookmarks(sendResponse) {
    ...
    try {
      ...
      this.notifyPopup('extractionStarted', { message: 'Starting bookmark extraction...' });
      ...
      if (result.errors.length > 0) {
        console.warn(`Processed ${result.successCount} bookmarks with ${result.errorCount} errors`);
      }

      // Check for duplicates
      const duplicateCheck = await this.storage.checkForDuplicates(result.bookmarks);

      // Save unique bookmarks
      if (duplicateCheck.unique.length > 0) {
        const saveSuccess = await this.storage.addBookmarks(duplicateCheck.unique);
