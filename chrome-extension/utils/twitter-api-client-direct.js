...
class TwitterAPIClientDirect {
  ...
  extractBookmarksFromStorage() {
    ...
    try {
      ...
      for (const key of localKeys) {
        if (key.includes('bookmark') || key.includes('timeline') || key.includes('tweet')) {
          try {
      ...
      for (const sessionKey of sessionKeys) {
        if (sessionKey.includes('bookmark') || sessionKey.includes('timeline') || sessionKey.includes('tweet')) {
          try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (bookmarks.length > 0) {
              console.log(`Successfully fetched ${bookmarks.length} bookmarks from ${endpoint}`);
              return bookmarks;
            }
          }
        } catch (error) {
  ...
  checkSessionStorage() {
    try {
      const sessionKeys = Object.keys(sessionStorage);
      const sessionIndicators = ['auth_token', 'ct0', 'guest_id', 'personalization_id', 'twitter_session', 'user_id'];
  ...
  checkLocalStorage() {
    try {
      const localKeys = Object.keys(localStorage);
      const sessionIndicators = ['auth_token', 'ct0', 'guest_id', 'personalization_id', 'twitter_session', 'user_id', 'twitter_ads_id'];
  ...
  checkCookies() {
    try {
      ...
      const sessionCookies = ['auth_token', 'ct0', 'guest_id', 'personalization_id', 'twitter_session'];
