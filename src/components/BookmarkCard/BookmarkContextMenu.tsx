...
import { LuFolder, LuStar, LuTrash2, LuShare2, LuExternalLink, LuCopy } from 'react-icons/lu'
...
const MenuItem = memo<MenuItemProps>(({ icon, label, onClick, color, isDanger }) => (
  <HStack
    px={4}
    py={3}
    cursor="pointer"
    bg="transparent"
    _hover={{ bg: isDanger ? 'rgba(239, 68, 68, 0.1)' : 'var(--color-bg-tertiary)' }}
    onClick={onClick}
    transition="background-color 0.2s ease"
    gap={3}
  >
    <Box color={color || (isDanger ? 'var(--color-error)' : 'var(--color-text-secondary)')}>
      {icon}
    </Box>
    <Text
      fontSize="sm"
...
export const BookmarkContextMenu = memo<BookmarkContextMenuProps>(({
  ...
  const isStarred = (bookmark as any).isStarred || (bookmark as any).is_starred || false
