...
const BookmarkFooter = memo(
  ({ bookmark, isInBulkMode }: BookmarkFooterProps) => {
    ...
    const getMetrics = () => {
      if (bookmark.metadata && 'engagement' in bookmark.metadata) {
        const engagement = bookmark.metadata.engagement
        if (engagement && typeof engagement === 'object' && engagement !== null) {
          return {
            likes: formatCount((engagement as { likes?: number; retweets?: number; replies?: number }).likes || 0),
            retweets: formatCount((engagement as { likes?: number; retweets?: number; replies?: number }).retweets || 0),
            replies: formatCount((engagement as { likes?: number; retweets?: number; replies?: number }).replies || 0),
          }
        }
      }
