...
export const CollectionTreeItem = memo<CollectionTreeItemProps>(({
  ...
  const [{ isOverBookmark, canDropBookmark, dragItem }, dropBookmark] = useDrop(() => ({
  ...
  const [{ isOverCollection, canDropCollection }, dropCollection] = useDrop(() => ({
    ...
    drop: async (item: any) => {
      ...
      try {
        ...
        toast.success(`Moved "${item.collection.name}" under "${collection.name}"`)
      } catch (error: any) {
    ...
    canDrop: (item: any) => {
      ...
      if (wouldCreateCircularReference(item.id, collection.id, collections)) {
        return false
      }
  ...
  }, [isOverBookmark, canDropBookmark, collection.id, collection.name, collection.isSmartCollection, dragItem])
  ...
  const isDropZone = (isOverBookmark && canDropBookmark) || (isOverCollection && canDropCollection)
  const isInvalidDrop = (isOverBookmark && !canDropBookmark) || (isOverCollection && !canDropCollection)
      ...
      >
        <HStack justify="space-between" align="center">
          ...
          >
            ...
            {(!hasChildren || isAtMaxDepth) && !collection.isSmartCollection && <Box w="14px" />}
