...
import { Box, HStack, VStack, Text, Button, Wrap, Spinner } from '@chakra-ui/react'
...
const SmartTagSuggestionInline = memo(({
  ...
  const hasMoreSuggestions = availableSuggestions.length > maxVisibleSuggestions
      ...
      >
        <HStack justify="space-between" align="center">
          <HStack gap={1.5}>
            ...
            <Text fontSize="12px" fontWeight={600} color="var(--color-text-secondary)">
              Smart Suggestions
            </Text>
            {!hasInteracted && (
              <Text fontSize="11px" color="var(--color-text-tertiary)" fontStyle="italic">
                Click to load
              </Text>
            )}
            {isLoading && (
            ...
            {hasInteracted && !isLoading && availableSuggestions.length > 0 && (
        ...
        >
          <VStack align="stretch" gap={2}>
                ...
                <Wrap gap={2}>
                  {visibleSuggestions.map((suggestion) => (
                    <Box
                      ...
                      transform={appliedTags.has(suggestion.tag) ? 'scale(0.9)' : 'scale(1)'}
            ...
            {hasMoreSuggestions && (
              ...
              >
                <HStack gap={1}>
                  ...
                  <Text>
                    {isExpanded ? 'Show Less' : `Show ${availableSuggestions.length - maxVisibleSuggestions} More`}
                  </Text>
                </HStack>
              </Button>
            )}
            ...
            {isExpanded && availableSuggestions.length > 0 && (
              <VStack align="stretch" gap={2} mt={2} pt={2} borderTop="1px solid var(--color-border)">
                <Text fontSize="11px" fontWeight={600} color="var(--color-text-tertiary)">
                  Why these tags?
                </Text>
                {visibleSuggestions.slice(0, 5).map((suggestion) => (
                  <HStack key={suggestion.tag} gap={2} align="start">
                    <Text fontSize="11px" fontWeight={600} color="var(--color-text-secondary)">
                      {suggestion.tag}:
                    </Text>
                    <Text fontSize="11px" color="var(--color-text-tertiary)" flex={1}>
