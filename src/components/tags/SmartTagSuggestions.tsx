      ...
      try {
        ...
        selectedBookmarks.forEach((bookmark) => {
          ...
          if (bookmark.domain) {
            domainMap.set(
              bookmark.domain,
              (domainMap.get(bookmark.domain) || 0) + 1
            )
          }
        ...
        const allBookmarksExceptSelected = bookmarks.filter(
          (bookmark) => !selectedBookmarkIds.includes(bookmark.id)
        )
        ...
        selectedBookmarks.forEach((selectedBookmark) => {
          allBookmarksExceptSelected.forEach((otherBookmark) => {
            ...
            // Author similarity
            if (
              selectedBookmark.author &&
              otherBookmark.author &&
            ...
            const selectedWords = (selectedBookmark.title || '')
              .toLowerCase()
              .split(/\s+/)
            const otherWords = (otherBookmark.title || '')
              .toLowerCase()
              .split(/\s+/)
            const commonWords = selectedWords.filter(
            ...
            if (similarity >= 3) {
              otherBookmark.tags?.forEach((tag) => {
                if (!existingTags.has(tag)) {
                  similarityMap.set(
                    tag,
                    (similarityMap.get(tag) || 0) + similarity
                  )
                }
              })
            }
        ...
        Array.from(trendingTags.entries())
          ...
          .forEach(([tag, count]) => {
            if (count >= 2) {
              // Only suggest if used at least twice recently
              suggestions.push({
                tag,
                score: count * 4,
                reason: 'trending',
