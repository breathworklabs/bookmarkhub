...
describe('bookmarkStore - Extended Actions', () => {
  ...
  describe('saved filter presets', () => {
    it('should save filter preset', () => {
      ...
      expect(presets[0].filters.selectedTags).toEqual(['react'])
      expect(presets[0].filters.searchQuery).toBe('test')
    ...
    it('should delete filter preset', () => {
      ...
      useBookmarkStore.getState().deleteFilterPreset('preset-1')

      expect(useBookmarkStore.getState().savedFilterPresets).toHaveLength(0)
    })
    ...
    it('should load filter preset', () => {
      ...
      useBookmarkStore.setState({ savedFilterPresets: [preset] })
      useBookmarkStore.getState().loadFilterPreset('preset-1')

      const state = useBookmarkStore.getState()
      expect(state.selectedTags).toEqual(['react', 'typescript'])
      expect(state.searchQuery).toBe('test query')
      expect(state.authorFilter).toBe('John')
      expect(state.domainFilter).toBe('example.com')
      expect(state.contentTypeFilter).toBe('article')
      expect(state.dateRangeFilter).toEqual({ type: 'month' })
      expect(state.quickFilters).toEqual(['starred', 'unread'])
    })
  })
})
