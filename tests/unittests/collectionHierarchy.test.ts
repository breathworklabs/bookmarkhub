...
describe('Collection Hierarchy Utilities', () => {
  ...
  describe('flattenCollectionTree', () => {
    it('should flatten a tree structure back to array', () => {
      const tree = buildCollectionTree(mockCollections, null, 0, ['work', 'projects', '2024'])
  ...
  describe('wouldCreateCircularReference', () => {
    it('should detect self-reference', () => {
      expect(wouldCreateCircularReference('work', 'work', mockCollections)).toBe(true)
    })

    it('should detect circular reference (parent as child)', () => {
      expect(wouldCreateCircularReference('work', 'projects', mockCollections)).toBe(true)
      expect(wouldCreateCircularReference('work', 'q1', mockCollections)).toBe(true)
    })

    it('should allow valid parent assignments', () => {
      expect(wouldCreateCircularReference('personal', 'work', mockCollections)).toBe(false)
      expect(wouldCreateCircularReference('q1', 'personal', mockCollections)).toBe(false)
    })
  })
  ...
  describe('validateParentAssignment', () => {
    ...
    it('should reject non-existent parent', () => {
      const result = validateParentAssignment('projects', 'nonexistent', mockCollections)
      expect(result.valid).toBe(false)
      expect(result.error).toContain('not found')
    })

    it('should reject circular reference', () => {
      const result = validateParentAssignment('work', 'projects', mockCollections)
      expect(result.valid).toBe(false)
      expect(result.error).toContain('circular')
    })

    it('should reject smart collection as parent', () => {
      const result = validateParentAssignment('personal', 'starred', mockCollections)
      expect(result.valid).toBe(false)
      expect(result.error).toContain('Smart collections')
    })

    it('should reject smart collection being nested', () => {
      const result = validateParentAssignment('starred', 'work', mockCollections)
      expect(result.valid).toBe(false)
      expect(result.error).toContain('Smart collections')
    })
    ...
    it('should reject exceeding max depth', () => {
      ...
      const result = validateParentAssignment('new-collection', 'q1', mockCollections, 3)
      expect(result.valid).toBe(false)
      expect(result.error).toContain('maximum nesting depth')
    })

    it('should allow valid parent assignment', () => {
      const result = validateParentAssignment('new-collection', 'work', mockCollections)
      expect(result.valid).toBe(true)
    })
  })
  ...
  describe('findCollectionByPath', () => {
    it('should find collection by path string', () => {
      const collection = findCollectionByPath('Work/Projects/2024', mockCollections)
    ...
    it('should return null for non-existent path', () => {
      const collection = findCollectionByPath('Work/Nonexistent', mockCollections)
