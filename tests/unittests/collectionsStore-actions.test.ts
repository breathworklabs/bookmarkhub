...
describe('collectionsStore actions', () => {
  ...
  it('createCollection adds a collection', async () => {
    ...
    vi.spyOn(localStorageService, 'createCollection').mockResolvedValueOnce(
      created
    )
  ...
  it('updateCollection updates name/description', async () => {
    ...
    await useCollectionsStore
      .getState()
      .updateCollection('x', { name: 'Updated', description: 'Desc' })
