...
describe('DataProcessingService', () => {
  describe('extractDomain', () => {
    it('should extract domain from valid URL', () => {
      expect(
        DataProcessingService.extractDomain('https://example.com/path')
      ).toBe('example.com')
      expect(DataProcessingService.extractDomain('https://test.org')).toBe(
        'test.org'
      )
      expect(
        DataProcessingService.extractDomain('http://subdomain.example.com')
      ).toBe('subdomain.example.com')
    })

    it('should remove www prefix', () => {
      expect(
        DataProcessingService.extractDomain('https://www.example.com')
      ).toBe('example.com')
      expect(
        DataProcessingService.extractDomain('http://www.test.org/page')
      ).toBe('test.org')
    })

    it('should handle URLs with ports', () => {
      expect(
        DataProcessingService.extractDomain('https://example.com:8080')
      ).toBe('example.com')
      expect(DataProcessingService.extractDomain('http://localhost:3000')).toBe(
        'localhost'
      )
    })
    ...
    it('should handle URLs with query parameters', () => {
      expect(
        DataProcessingService.extractDomain('https://example.com?param=value')
      ).toBe('example.com')
      expect(
        DataProcessingService.extractDomain('https://test.org/path?a=1&b=2')
      ).toBe('test.org')
    })

    it('should handle URLs with hash fragments', () => {
      expect(
        DataProcessingService.extractDomain('https://example.com#section')
      ).toBe('example.com')
      expect(
        DataProcessingService.extractDomain('https://test.org/page#top')
      ).toBe('test.org')
    })
    ...
    it('should return "unknown" for invalid URLs', () => {
      ...
      expect(DataProcessingService.extractDomain('ftp://invalid')).toBe(
        'invalid'
      )
    })
    ...
    it('should handle special characters in domain', () => {
      expect(
        DataProcessingService.extractDomain('https://test-domain.com')
      ).toBe('test-domain.com')
      expect(
        DataProcessingService.extractDomain('https://sub.domain.example.com')
      ).toBe('sub.domain.example.com')
    })
  })

  describe('validateUrl', () => {
    it('should validate correct URLs', () => {
      expect(DataProcessingService.validateUrl('https://example.com')).toBe(
        true
      )
      expect(DataProcessingService.validateUrl('http://test.org')).toBe(true)
      expect(
        DataProcessingService.validateUrl('https://www.example.com/path')
      ).toBe(true)
      expect(
        DataProcessingService.validateUrl('https://example.com:8080')
      ).toBe(true)
    })
    ...
    it('should validate URLs with query parameters', () => {
      expect(
        DataProcessingService.validateUrl('https://example.com?param=value')
      ).toBe(true)
      expect(
        DataProcessingService.validateUrl('https://example.com?a=1&b=2&c=3')
      ).toBe(true)
    })

    it('should validate URLs with hash fragments', () => {
      expect(
        DataProcessingService.validateUrl('https://example.com#section')
      ).toBe(true)
      expect(
        DataProcessingService.validateUrl('https://example.com/page#top')
      ).toBe(true)
    })
    ...
    it('should validate localhost URLs', () => {
      expect(DataProcessingService.validateUrl('http://localhost:3000')).toBe(
        true
      )
      expect(DataProcessingService.validateUrl('http://127.0.0.1:8080')).toBe(
        true
      )
    })
    ...
    it('should validate URLs with encoded characters', () => {
      expect(
        DataProcessingService.validateUrl(
          'https://example.com/path%20with%20spaces'
        )
      ).toBe(true)
      expect(
        DataProcessingService.validateUrl(
          'https://example.com/path?query=%E2%9C%93'
        )
      ).toBe(true)
    })
  })
  ...
  describe('processImages', () => {
    ...
    it('should handle images with both _normal and _bigger in filename', () => {
      ...
      expect(result.normalProfileImages).toEqual([
        'https://pbs.twimg.com/profile_images/test_normal_bigger.jpg',
      ])
      expect(result.biggerProfileImages).toEqual([
        'https://pbs.twimg.com/profile_images/test_normal_bigger.jpg',
      ])
