...
const localStorageBookmarks = localStorage.getItem(
...
if (localStorageBookmarks) {
  try {
    ...
    if (parsed.length > 0) {
      ...
      const isImportedData = importedTitles.some((title) =>
        firstBookmark.title.includes(title.split(' ')[0])
      )
      ...
      const archived = parsed.filter(
        (b) => b.is_archived === true || b.is_archived === 'true'
      )
...
try {
  ...
  const storeKeys = Object.keys(window).filter(
    (key) =>
      key.includes('store') ||
...
console.log(
  '2. If localStorage has data but app shows 6 archived → the app is using mock data'
)
console.log(
  '3. Check browser console for "using mock data" vs "loaded from localStorage" messages'
)
