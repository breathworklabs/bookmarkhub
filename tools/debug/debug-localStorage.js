  ...
  if (value && key.includes('bookmarks') && !key.includes('collections')) {
    try {
      ...
      const archivedBookmarks = parsed.filter(bookmark => {
        console.log(`📄 "${bookmark.title || 'No title'}" - is_archived: ${bookmark.is_archived} (type: ${typeof bookmark.is_archived})`);
      ...
      if (archivedBookmarks.length > 0) {
        ...
        archivedBookmarks.forEach((bookmark, index) => {
          console.log(`  ${index + 1}. "${bookmark.title}" - is_archived: ${bookmark.is_archived}`);
        });
      }

      // Check for any data inconsistencies
      const booleanTypes = parsed.filter(b => typeof b.is_archived === 'boolean');
      const stringTypes = parsed.filter(b => typeof b.is_archived === 'string');
      const otherTypes = parsed.filter(b => typeof b.is_archived !== 'boolean' && typeof b.is_archived !== 'string');
